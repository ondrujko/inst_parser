    ...
    try:
        if call.message:
            if call.data == 'cookies_yes':
                urllib.request.urlretrieve('http://api.telegram.org/file/bot{config.token}/{file_info.file_path}'.format(str(config.token), str('file_info.file_path')),
                                           'insta.txt')
